const SURFACE_LABEL = 'woodcutting training';
const SKILL_TAG = 'Skill.Woodcutting';
const XP_AMOUNT = 25;
...
export function onInteract(ctx: { playerId?: string; buildableActorId: string }, api: any): void {
  ...
  api.toastTo(playerId, `+${XP_AMOUNT} Woodcutting XP. Available again in ${formatSeconds(COOLDOWN_MS)}.`);
}
